...
export class RutaDetalleComponent implements OnInit {
  ...
  todo() {
    this.rutaService.detalleRutaById(this.idRuta).subscribe(
  ...
  enviarUbicacionesMapa(detalleRuta: any) {
    ...
    if (Array.isArray(entregas) && entregas.length > 0) {
      this.puntosClientesUbicaciones = entregas
        .map((entrega) => {
  ...
  agregarClienteEnRuta() {
    ...
    this.rutaService
      .addPuntoEntregaRutaById(this.idRuta, newPuntosDeEntrega)
      .subscribe(
        (response) => {
          this.visible = false;
  ...
  onMunicipioSelectionChange(event: any) {
    ...
    for (let municipios of this.allMunicipioXEstado) {
      if (
        municipios
          .toLowerCase()
          .indexOf(this.selectedMunicipio.toLowerCase()) == 0
      ) {
