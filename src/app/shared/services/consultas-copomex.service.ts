...
export class ConsultasCOPOMEXService {
  ...
  getColoniaXMunicipio(idPurificadora: string): Observable<any> {
    // http://localhost:4000/direccion/coloniasByMunicipio/Huejutla de Reyes
    const url =
      `${environment.api}/direccion/coloniasByMunicipio/` + idPurificadora;
    return this.http.get(url);
  }
  getColoniaXMunicipioByClientes(idPurificadora: any): Observable<any> {
    const url = `${environment.api}/direccion/getColoniasByMunicipioByClientes/`;
    return this.http.post(url, { idPurificadora });
  }
